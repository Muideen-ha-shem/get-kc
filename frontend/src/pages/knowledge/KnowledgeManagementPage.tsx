  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-3">
        <h2 className="text-sm font-semibold text-gray-500 uppercase">Sources</h2>
        <button onClick={() => setShowCreate((v) => !v)} className="text-xs bg-blue-600 text-white rounded px-3 py-2">
          + New Source
        </button>
      </div>

      {showCreate && (
        <form onSubmit={createSource} className="bg-white border rounded p-4 mb-4 flex flex-col gap-3 max-w-lg">
          <label className="text-sm">
            Type
            <select value={sourceType} onChange={(e) => setSourceType(e.target.value as SourceType)} className="w-full border rounded px-2 py-1 mt-1">
              {SOURCE_TYPES.filter((t) => t !== 'faq').map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </label>
          <label className="text-sm">
            Name
            <input value={name} onChange={(e) => setName(e.target.value)} required className="w-full border rounded px-2 py-1 mt-1" />
          </label>
          {sourceType === 'website' && (
            <>
              <label className="text-sm">
                Website URL
                <input value={url} onChange={(e) => setUrl(e.target.value)} required className="w-full border rounded px-2 py-1 mt-1" />
              </label>
              <label className="text-sm">
                Crawl depth
                <input type="number" min={1} max={5} value={maxDepth} onChange={(e) => setMaxDepth(Number(e.target.value))} className="w-full border rounded px-2 py-1 mt-1" />
              </label>
              <label className="text-sm">
                Include paths (comma-separated, optional)
                <input value={includePaths} onChange={(e) => setIncludePaths(e.target.value)} className="w-full border rounded px-2 py-1 mt-1" />
              </label>
              <label className="text-sm">
                Exclude paths (comma-separated, optional)
                <input value={excludePaths} onChange={(e) => setExcludePaths(e.target.value)} className="w-full border rounded px-2 py-1 mt-1" />
              </label>
              <label className="text-sm">
                Schedule
                <select value={schedule} onChange={(e) => setSchedule(e.target.value as Schedule)} className="w-full border rounded px-2 py-1 mt-1">
          ...
          <label className="text-sm">
            Collection (optional)
            <select value={collectionId} onChange={(e) => setCollectionId(e.target.value)} className="w-full border rounded px-2 py-1 mt-1">
          ...
          <button type="submit" className="bg-blue-600 text-white rounded px-4 py-2 text-sm self-start">Create</button>
        </form>
      )}
      ...
      <div className="bg-white border rounded divide-y">
        {sources.map((s) => (
          <div key={s.id} className="flex items-center justify-between px-4 py-3">
            <div>
              <p className="text-sm font-medium">{s.name} <span className="text-xs text-gray-400">· {s.source_type} · {s.status}</span></p>
              <p className="text-xs text-gray-500">{s.schedule} {s.last_crawled_at ? `· last crawled ${s.last_crawled_at.slice(0, 19).replace('T', ' ')}` : ''}</p>
            </div>
            <div className="flex gap-2">
              {s.source_type === 'website' && (
                <button onClick={() => recrawl(s.id)} className="text-xs bg-gray-200 rounded px-2 py-1">Recrawl</button>
              )}
              {s.status === 'paused' ? (
                <button onClick={() => resume(s.id)} className="text-xs bg-green-100 text-green-700 rounded px-2 py-1">Resume</button>
              ) : (
                <button onClick={() => pause(s.id)} className="text-xs bg-gray-200 rounded px-2 py-1">Pause</button>
              )}
              <button onClick={() => remove(s.id)} className="text-xs bg-red-100 text-red-700 rounded px-2 py-1">Archive</button>
            </div>
          </div>
        ))}
        {sources.length === 0 && <p className="px-4 py-3 text-sm text-gray-400">No sources yet.</p>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="max-w-md">
      ...
      <form onSubmit={submit} className="bg-white border rounded p-4 flex flex-col gap-3">
        <label className="text-sm">
          Source
          <select value={sourceId} onChange={(e) => setSourceId(e.target.value)} required className="w-full border rounded px-2 py-1 mt-1">
            <option value="">Select a source…</option>
            {sources.map((s) => <option key={s.id} value={s.id}>{s.name} ({s.source_type})</option>)}
          </select>
        </label>
        <input type="file" onChange={(e) => setFile(e.target.files?.[0] ?? null)} required />
        <button type="submit" className="bg-blue-600 text-white rounded px-4 py-2 text-sm self-start">Upload</button>
      </form>
      {result && (
        <div className="mt-4 text-sm bg-green-50 border border-green-200 rounded p-3">
          <p className="font-medium">{result.title}</p>
          <p className="text-xs text-gray-600">Status: {result.status} · {result.chunk_count} chunks</p>
        </div>
      )}
    </div>
  );
...
function FaqTab({ workspaceId, collections }: { workspaceId: string; collections: KnowledgeCollection[] }) {
  ...
  return (
    <div className="max-w-lg">
      <form onSubmit={submit} className="bg-white border rounded p-4 flex flex-col gap-3 mb-4">
        <label className="text-sm">
          Question
          <input value={question} onChange={(e) => setQuestion(e.target.value)} required className="w-full border rounded px-2 py-1 mt-1" />
        </label>
        <label className="text-sm">
          Answer
          <textarea value={answer} onChange={(e) => setAnswer(e.target.value)} required rows={3} className="w-full border rounded px-2 py-1 mt-1" />
        </label>
        <label className="text-sm">
          Category (optional)
          <input value={category} onChange={(e) => setCategory(e.target.value)} className="w-full border rounded px-2 py-1 mt-1" />
        </label>
        <label className="text-sm">
          Collection (optional)
          <select value={collectionId} onChange={(e) => setCollectionId(e.target.value)} className="w-full border rounded px-2 py-1 mt-1">
        ...
        <button type="submit" className="bg-blue-600 text-white rounded px-4 py-2 text-sm self-start">Add FAQ</button>
      </form>
      ...
      <div className="bg-white border rounded divide-y">
        ...
        {faqs.length === 0 && <p className="px-4 py-3 text-sm text-gray-400">No FAQs yet.</p>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="max-w-md">
      <form onSubmit={submit} className="bg-white border rounded p-4 flex flex-col gap-3 mb-4">
        <label className="text-sm">
          Name
          <input value={name} onChange={(e) => setName(e.target.value)} required className="w-full border rounded px-2 py-1 mt-1" placeholder="Support, Sales, Finance…" />
        </label>
        <label className="text-sm">
          Description (optional)
          <input value={description} onChange={(e) => setDescription(e.target.value)} className="w-full border rounded px-2 py-1 mt-1" />
        </label>
        <button type="submit" className="bg-blue-600 text-white rounded px-4 py-2 text-sm self-start">Create Collection</button>
      </form>
      <div className="bg-white border rounded divide-y">
        {collections.map((c) => (
        ...
        {collections.length === 0 && <p className="px-4 py-3 text-sm text-gray-400">No collections yet.</p>}
      </div>
    </div>
  );
}
...
function TestingTab({ workspaceId }: { workspaceId: string }) {
  ...
  return (
    <div className="max-w-2xl">
      <p className="text-sm text-gray-600 mb-3">Reuses the exact retrieval pipeline the AI itself uses — this is what the customer would get back.</p>
      <form onSubmit={submit} className="flex gap-2 mb-4">
        <input value={question} onChange={(e) => setQuestion(e.target.value)} required className="flex-1 border rounded px-3 py-2 text-sm" placeholder="Ask a question…" />
        <button type="submit" className="bg-blue-600 text-white rounded px-4 py-2 text-sm">Test</button>
      </form>

      {result && (
        <div className="bg-white border rounded p-4">
          <p className="text-sm mb-2">Confidence: <strong>{(result.confidence * 100).toFixed(0)}%</strong></p>
          <div className="flex flex-col gap-3">
            {result.chunks.map((chunk, i) => (
              <div key={i} className="border rounded p-3">
                ...
                <p className="text-xs text-blue-600 mt-1">{chunk.url}</p>
              </div>
            ))}
            {result.chunks.length === 0 && <p className="text-sm text-gray-400">No chunks retrieved.</p>}
          </div>
        </div>
      )}
    </div>
...
function QualityTab({ workspaceId }: { workspaceId: string }) {
  ...
  return (
    <div className="max-w-2xl">
      <button onClick={runScan} className="bg-blue-600 text-white rounded px-4 py-2 text-sm mb-4">Run Quality Scan</button>
      {report ? (
        ...
        <p className="text-sm text-gray-400">No quality report yet — run a scan.</p>
      )}
    </div>
  );
}
      ...
      <tbody>
        {jobs.map((j) => (
          <tr key={j.id} className="border-b last:border-0">
            ...
            <td className="px-4 py-2 text-gray-500">{j.started_at?.slice(0, 19).replace('T', ' ') ?? '—'}</td>
          </tr>
        ))}
      </tbody>
    </table>
  ...
  if (forbidden) {
    return (
      <div className="p-8 text-center">
        <p className="text-lg font-medium">You don't have admin access to this workspace's knowledge base.</p>
