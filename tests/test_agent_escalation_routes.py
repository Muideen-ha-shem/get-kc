...
class TestAgentsRoutes:
    def test_agents_me_auto_provisions(self, client):
        _authenticate()
        with patch("src.api.routes.agents._agent_service.get_or_create", return_value=_agent()) as mock_get_or_create:
    ...
    def test_update_status_returns_404_when_not_an_agent(self, client):
        _authenticate()
        with patch("src.api.routes.agents._agent_service.get_by_auth_user_id", return_value=None):
    ...
    def test_update_status_updates_when_registered(self, client):
        _authenticate()
        with patch("src.api.routes.agents._agent_service.get_by_auth_user_id", return_value=_agent()), \
             patch("src.api.routes.agents._agent_service.update_status", return_value=_agent(status="available")):
            response = client.patch("/agents/status", json={"status": "available"})
