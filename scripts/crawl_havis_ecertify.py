"""Crawl Havis eCertify (https://ha-shem.com/HAVIS-360/havis-ecertify/) into
the crawled_pages table.
...
Havis eCertify is a single product page on the main ha-shem.com site, so
this crawls only that page (``max_depth=0``, no link-following) rather than
...
NOT executed automatically — run manually: `python -m scripts.crawl_havis_ecertify`
"""
...
if __name__ == "__main__":
    asyncio.run(
        crawl_site(
            PRODUCT_SITES["Havis eCertify"]["url"],
            max_depth=0,
            headless=True,
        )
    )
